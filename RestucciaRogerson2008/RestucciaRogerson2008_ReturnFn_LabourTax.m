function F=RestucciaRogerson2008_ReturnFn_LabourTax(aprime_val, a_val, s_val, tau_val,w,r,alpha,gamma,taurate, subsidyrate,cf)
...
% Physical capital:
kbar=(alpha/r)^((1-gamma)/(1-gamma-alpha)) *(gamma/((1+tau)*w))^(gamma/(1-gamma-alpha)) *s_val^(1/(1-alpha-gamma));

% Labour
nbar=((s_val*gamma)/((1+tau)*w))^(1/(1-gamma)) *kbar^(alpha/(1-gamma));

% Substitute kbar, nbar in pi and compute W(s,kbar(s,theta))
pibar=s_val*(kbar^alpha)*(nbar^gamma)-(1+tau)*w*nbar-r*kbar-cf;
