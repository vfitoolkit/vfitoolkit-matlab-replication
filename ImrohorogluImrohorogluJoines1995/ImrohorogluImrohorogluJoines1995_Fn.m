...
FnsToEvaluate_6 = @(aprime_val,a_val,z_val,r,A,alpha,delta,h,I_j,epsilon_j) ((1-alpha)*(A^(1/(1-alpha)))*((r+delta)/alpha)^(alpha/(alpha-1)))*I_j*epsilon_j*(1-z_val); % Potential Labour income (in worked 100 percent of time) of the unemployed (the unemployment benefits are set as a fraction, zeta, of this)
...
FnsToEvaluateParamNames2(3).Names={'r','tau_u', 'tau_s','h','zeta','epsilon_j','I_j','alpha','delta', 'A','SSdivw', 'Tr_beq'}; 
FnsToEvaluate_C = @(aprime_val,a_val,z_val,r,tau_u, tau_s,h,zeta,epsilon_j,I_j,alpha,delta, A,SSdivw, Tr_beq) ImrohorogluImrohorogluJoines1995_ConsumptionFn(aprime_val,a_val,z_val, r,tau_u, tau_s,h,zeta,epsilon_j,I_j,alpha,delta, A,SSdivw, Tr_beq); % Consumption
