function F=GuerrieriLorenzoni2017_DurableGoods_ReturnFn(d1, d2, bprime, b, k, z, r, alpha, gamma, psi, eta, phi_k, delta, zeta, chi, v, B, Bprime, omega)
% d2 is defined as the change in k, so d2=kprime-k or equivalently kprime=d2+k

F=-Inf;

% Guerrieri & Lorenzoni (2017) refer to: d1 as n_it, bprime_val as b_it+1, 
...
% gk=(kprime-k+delta*k)*(kprime>=k)+((1-zeta)*(kprime-k)+delta*k)*(kprime<k);
gk=(d2+delta*k)*(d2>=0)+((1-zeta)*d2+delta*k)*(d2<0);
...
c=b+z*d1-tau_tilde-(1-chi)*(bprime<0)*(1/(1+r))*bprime-(bprime>0)*(1/(1+r))*bprime-gk; % q=1/(1+r)
...
if c>0 && k>0
    ...
    F=(((c^alpha)*(k^(1-alpha)))^(1-gamma))/(1-gamma)+(1/(1-omega))*psi*((1-d1)^(1-eta))/(1-eta);
end

% Impose borrowing constraint (now a collateral constraint)
if bprime<(-phi_k*(d2+k))
    F=-Inf;
end
