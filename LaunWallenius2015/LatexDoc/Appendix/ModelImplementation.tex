...
The encode/decode arithmetic is duplicated inline in \verb|LaunWallenius2015_aprimeFn.m| and \verb|LaunWallenius2015_ReturnFn.m|. Factoring it into a helper file would be slightly cleaner but would prevent toolkit-side inlining --- both of those functions are on the inner loop of the value-function iteration and called millions of times.
...
\end{itemize}
Eligibility constraints are enforced inside the return function (\verb|LaunWallenius2015_ReturnFn.m|) by returning $-\infty$ for infeasible $(d, \text{state})$ pairs; the aprimeFn just executes the formal transition without checking eligibility, knowing that infeasible-action paths will be excluded by $-\infty$ utilities.
...
vfoptions.aprimeFn = @(d, hist_idx, agej, agej_pb_first, agej_di_max) ...
                     LaunWallenius2015_aprimeFn(d, hist_idx, agej, ...
...
\begin{itemize}
\item \textbf{Wage profiles.} The four life-cycle earnings profiles in LW2015 Fig.~1 (LINDA 1968--1997) are estimated by regression on age, age$^2$, and individual fixed effects, but the regression coefficients are not printed in the paper, the SSE working-paper version (WP 741), or any online appendix or replication package we can locate; nor can they be recovered from the LW2016 sequel replication code (RED 14-161), which publishes only \emph{two} regressed wage profiles. The original regression coefficients were kindly sent to us by Tobias Laun via email and we use them directly, so the wage profiles are no longer a departure from the paper; see Section~\ref{sec:wage-profiles} for the coefficients.
