...
%
%
% NOTE: the wage profiles (Params.w_c0_i, Params.w_c1_i, Params.w_c2_i) use the original LINDA
% regression coefficients, which were sent to us by Tobias Laun via email. See inline comments for details.
...
% Inversion (decode) and encoding are inlined in both LaunWallenius2015_aprimeFn.m
% and LaunWallenius2015_ReturnFn.m (look for `n_no_di = 1197`). The agent's
...
% the fly by LaunWallenius2015_PB.m, _DI.m, _OPB.m at every ReturnFn call,
% which in turn call LaunWallenius2015_AP.m to evaluate average pension points
...
%% Wage profiles, per-PType (w_c0, w_c1, w_c2) scalars
% Quadratic in real age, in BA units per year of full-time work. Closed
% form is inlined where used (ReturnFn for current-period earnings; AP,
% OPB, OPB_DI for per-age wages):
%   w(real_age) = w_c2_i*real_age^2 + w_c1_i*real_age + w_c0_i
%
% The LINDA 1968-1997 regression coefficients were sent to us by Tobias
% Laun via email (the original coefficients are not published in the
% paper or any working-paper appendix). The emailed w.m function is:
%   c0=[-1.155144 3.79198];     constant component  [NC, C]
%   c1=[0.2441215 0.1907433];   linear component
%   c2=[-0.0022909 -0.0019136]; quadratic component
%   fe=[-1.537212 -2.682249;    fixed effect [NC low,  C low;
%        1.093206  2.419877];                 NC high, C high]
%   w(age) = c2(edu)*age^2 + c1(edu)*age + c0(edu) + fe(pro,edu)
% Below, the within-education fixed effect is folded into the per-type
% constant: w_c0_i = c0(edu) + fe(pro,edu).
Params.w_c0_i=[-2.692356; -0.061938; 1.109731; 6.211857];   % NCL, NCH, CL, CH
Params.w_c1_i=[0.2441215; 0.2441215; 0.1907433; 0.1907433];
Params.w_c2_i=[-0.0022909; -0.0022909; -0.0019136; -0.0019136];
...
for ii=1:4
    fig1_w(ii,:)=Params.w_c2_i(ii)*fig1_ages.^2+Params.w_c1_i(ii)*fig1_ages+Params.w_c0_i(ii);
end
fig1=figure(1);
...
grid on;
saveas(fig1,'SavedOutput/Graphs/LaunWallenius2015_Figure1.png');

%% Table 4: calibrated parameter values (LW2015 Table 4)
fid=fopen('SavedOutput/LatexInputs/LaunWallenius2015_Table4.tex','w');
...
vfoptions.aprimeFn=@(d,hist_idx,agej,agej_pb_first,agej_di_max) LaunWallenius2015_aprimeFn(d,hist_idx,agej,agej_pb_first,agej_di_max);
...
ReturnFn=@(d,kprime,k,hist_idx,h,agej,r,b1,b2,b3,b4,b5,tax_thresh,tau_l1,tau_l2,tau_ss,tau_c,T,w_c0_i,w_c1_i,w_c2_i,collar_i,lbar,ppmax,N_j,agejshifter,agej_pb_first,agej_di_max,agej_65,regime) ...
    LaunWallenius2015_ReturnFn(d,kprime,k,hist_idx,h,agej,r,b1,b2,b3,b4,b5,tax_thresh,tau_l1,tau_l2,tau_ss,tau_c,T,w_c0_i,w_c1_i,w_c2_i,collar_i,lbar,ppmax,N_j,agejshifter,agej_pb_first,agej_di_max,agej_65,regime);
...
if doPart(1)==1
    ...
    save('./SavedOutput/LaunWallenius2015_results.mat','V_pre','Policy_pre','StationaryDist_pre','AllStats_pre','AgeConditionalStats_pre','Params','-v7.3');
else
    load ./SavedOutput/LaunWallenius2015_results.mat
end
% Free up some memory
...
grid on;
saveas(fig3,'SavedOutput/Graphs/LaunWallenius2015_Figure3.png');
...
grid on;
saveas(fig4,'SavedOutput/Graphs/LaunWallenius2015_Figure4.png');
...
if doPart(2)==1
    ...
    save('./SavedOutput/LaunWallenius2015_results_partialeqm.mat','V_post','Policy_post','StationaryDist_post','AllStats_post','AgeConditionalStats_post','Params','-v7.3');
else
    load ./SavedOutput/LaunWallenius2015_results_partialeqm.mat
end
% Free up some memory
...
if doPart(3)==1
    ...
    % FnsToEvaluate: per-agent budget surplus (helper in LaunWallenius2015_BudgetFn.m)
    FnsToEvaluate_GE.surplus=@(d,kprime,k,hist_idx,h,agej,r,tax_thresh,tau_l1,tau_l2,tau_ss,tau_c,T,w_c0_i,w_c1_i,w_c2_i,collar_i,lbar,ppmax,N_j,agejshifter,agej_65,regime) ...
        LaunWallenius2015_BudgetFn(d,kprime,k,hist_idx,h,agej,r,tax_thresh,tau_l1,tau_l2,tau_ss,tau_c,T,w_c0_i,w_c1_i,w_c2_i,collar_i,lbar,ppmax,N_j,agejshifter,agej_65,regime);
    ...
    save('./SavedOutput/LaunWallenius2015_results_generaleqm.mat','V_ge','Policy_ge','StationaryDist_ge','AllStats_ge','AgeConditionalStats_ge','Params','-v7.3');
else
    load ./SavedOutput/LaunWallenius2015_results_generaleqm.mat
end
% Free up some memory
...
grid on;
saveas(fig5b,'SavedOutput/Graphs/LaunWallenius2015_Figure5b.png');
...
if doPart(4)==1
    ...
    save('./SavedOutput/LaunWallenius2015_results_fullreform.mat','V_full','Policy_full','StationaryDist_full','AllStats_full','AgeConditionalStats_full','Params','-v7.3');
else
    load ./SavedOutput/LaunWallenius2015_results_fullreform.mat
end
% Free up some memory
...
if doPart(5)==1
    ...
    save('./SavedOutput/LaunWallenius2015_results_fullreform_ge.mat','V_full_ge','Policy_full_ge','StationaryDist_full_ge','AllStats_full_ge','AgeConditionalStats_full_ge','Params','T_full_ge','-v7.3');
else
    load ./SavedOutput/LaunWallenius2015_results_fullreform_ge.mat
end
...
grid on;
saveas(fig5,'SavedOutput/Graphs/LaunWallenius2015_Figure5.png');
...
grid on;
saveas(fig6,'SavedOutput/Graphs/LaunWallenius2015_Figure6.png');
...
if doPart(6)==1
    load ./SavedOutput/LaunWallenius2015_results.mat V_pre
    load ./SavedOutput/LaunWallenius2015_results_fullreform_ge.mat V_full_ge
    ...
    save('./SavedOutput/LaunWallenius2015_results_cev.mat','CV','CV_pop_weighted','Gamma');
else
    load ./SavedOutput/LaunWallenius2015_results_cev.mat
end
