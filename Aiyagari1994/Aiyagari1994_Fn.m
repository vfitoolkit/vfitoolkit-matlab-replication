function OutputVector=Aiyagari1994_Fn(n_k,n_s,n_p,beta,alpha,delta,mu,sigma,rho,q,Parallel, tauchenoptions, mcmomentsoptions, vfoptions, simoptions)
...
[s_grid, pi_s]=TauchenMethod(0,(sigma^2)*(1-rho^2),rho,n_s,q,tauchenoptions); %[states, transmatrix]=TauchenMethod_Param(mew,sigmasq,rho,znum,q,Parallel,Verbose), transmatix is (z,zprime)
...
r_ss=1/beta-1;
K_ss=((r_ss+delta)/alpha)^(1/(alpha-1)); %The steady state capital in the absence of aggregate uncertainty.
...
r_grid=sort([linspace(-delta+0.0001,-0.0001,floor(1/3*n_p)),linspace(0,r_ss,ceil(2/3*n_p))])';
...
ReturnFnParams=[alpha,delta,mu,p_test]; %It is important that these are in same order as they appear in 'Aiyagari1994_ReturnFn'
...
% tic;
[p_eqm,p_eqm_index, MarketClearance]=HeteroAgentStationaryEqm_Case1_Par2(V0, n_d, n_a, n_s, n_p, pi_s, d_grid, a_grid, s_grid, p_grid, beta, ReturnFn, SSvaluesFn, SSvalueParams, MarketPriceEqns, MarketPriceParams, MultiMarketCriterion, simoptions, vfoptions,ReturnFnParams, IndexesForPricesInReturnFnParams);
% findeqmtime=toc
save ./SavedOutput/Aiyagari1994Market.mat p_eqm p_eqm_index MarketClearance

% Equilibrium wage
w=(1-alpha)*((p_eqm+delta)/alpha)^(alpha/(alpha-1));
...
ReturnFnParams(IndexesForPricesInReturnFnParams)=p_eqm;
[~,Policy]=ValueFnIter_Case1(V0, n_d,n_a,n_s,d_grid,a_grid,s_grid, pi_s, beta, ReturnFn,vfoptions,ReturnFnParams);
...
aggsavingsrate=delta*SSvalues_AggVars^(1-alpha);
...
GraphString=['Aiyagari1994_MarketClearance_mu', num2str(mu), 'rho', num2str(rho), 'sigma', num2str(sigma)];
% Create a graph displaying the market clearance
fig1=figure(1);
plot(p_grid,MarketClearance, 'x',p_grid, zeros(n_p,1),p_eqm, 0, 'r o')
title(['Market clearance: mu=', num2str(mu), ' rho=', num2str(rho), ' sigma=', num2str(sigma)],'FontSize',18);
...
export_fig(fig1, ...            % figure handle
    ['./Graphs/',GraphString,'.pdf'],... % name of output file without extension   %  '-painters', ...            % renderer
    '-pdf', ...                 % file format
    '-r300' );                  % resolution
