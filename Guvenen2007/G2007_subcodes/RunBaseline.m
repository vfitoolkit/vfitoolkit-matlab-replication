...
if exist('RunBaseline_diary.txt','file'); delete('RunBaseline_diary.txt'); end
if ~exist('SavedOutput/Graphs','dir'); mkdir('SavedOutput/Graphs'); end
diary('RunBaseline_diary.txt');
fprintf('=== RunBaseline.m run %s ===\n',char(datetime('now')));
npass=0; nfail=0;
...
ilam_base=3;
do_deltacalib=1;             % =1: bisect delta to W/Y=4 for each lambda (8 iterations)
...
amax=100*Params.Ybar_T; % 100x (was 60x): at the calibrated delta the top-3-point mass
                        % was borderline ~1e-5 for lambda=0,0.4; widening clears it
...
vfoptions.gridinterplayer=1;  % interpolate aprime between asset nodes (smooth policy -> the
vfoptions.ngridinterp=10;     % var(log c) rise converges without a huge n_a; see NOTES.md)
...
for ilam=1:length(lambdavec)
    ...
    fprintf('W/Y total-income = %.2f (target 4), delta=%.4f\n',WYtot,Params.delta);
...
fprintf('(paper Fig 8: lambda=0 ~0.32, lambda*=0.62 ~0.21 [matches US data], lambda=1 ~0.14)\n');
if all(diff(rises)<0); npass=npass+1; fprintf('rise in var(log c) decreasing in lambda: PASS\n');
else; nfail=nfail+1; fprintf('FAIL: rise not monotone decreasing in lambda\n'); end
if rises(ilam_base)>0.15 && rises(ilam_base)<0.30; npass=npass+1; else; nfail=nfail+1; fprintf('FAIL: baseline (lambda=0.62) rise off the ~0.21 target\n'); end
if rises(1)/rises(ilam_base)>1.3 && rises(1)/rises(ilam_base)<1.8; npass=npass+1; else; nfail=nfail+1; fprintf('FAIL: lambda=0 overshoot ratio off (paper ~32/21=1.5)\n'); end
if rises(end)>0.09 && rises(end)<0.19; npass=npass+1; else; nfail=nfail+1; fprintf('FAIL: lambda=1 rise off the ~0.14 target\n'); end
...
fig=figure(2); clf;
plot(agevec,results(ilam_base).meanc,'-','LineWidth',1.5); hold on;
plot(agevec,results(ilam_base).meany,'--','LineWidth',1.5);
xlabel('age'); legend('mean consumption','mean income','Location','best');
title('HIP baseline (\lambda=0.62): consumption and income');
saveas(fig,'SavedOutput/Graphs/RunBaseline_fig2_meancy.png');
fig=figure(3); clf; hold on;
for ilam=1:length(lambdavec)
    plot(agevec,results(ilam).meana,'LineWidth',1.5);
end
xlabel('age'); legend('\lambda=0','\lambda=0.4','\lambda=0.62','\lambda=1','Location','northeast');
title('Mean assets by \lambda');
saveas(fig,'SavedOutput/Graphs/RunBaseline_fig3_meanassets.png');
fig=figure(4); clf;
tvec=1:Jwork;
van=s2a_u+Params.sigma2_eps+Params.sigma2_eta*(1-Params.rho.^(2*tvec))/(1-Params.rho^2)+2*sab_u*(tvec-1)+s2b_u*(tvec-1).^2;
plot(25:64,results(ilam_base).varlogy(1:Jwork),'-o'); hold on; plot(25:64,van,'--','LineWidth',1.5);
xlabel('age'); legend('model (dist)','analytic HIP decomposition','Location','northwest');
title('var(log y): chain+types+dist vs theory (baseline \lambda)');
saveas(fig,'SavedOutput/Graphs/RunBaseline_fig4_varlogy.png');

%% Save and summarize
save('RunBaseline_results.mat','results','lambdavec','Params','a_grid','n_z','n_a','N_j','alphagrid','w_types','dbet','-v7.3');
