function RatioEarningsOldYoung=CDGRR2003_RatioEarningsOldYoung(NSims, StationaryDist, Policy, Params, simoptions, n_d,n_a,n_z, d_grid,a_grid,z_grid, pi_z)
% Note: Calculating this 'specific ages' statistic in an infinite-horizon model is an awkward idea as it takes forever to run.  
...
K=3; % I am going to simulate 'K' times as many people as needed, and then just keep all of those who do not retire
...
StationaryDistKron=reshape(StationaryDist,[N_a,N_z]);
...
% sample).
IndividualEarnings_Young=zeros(K*NSims,1); %age 26-30
IndividualEarnings_Old=zeros(K*NSims,1);   %age 46-50
cumsumpi_s=cumsum(pi_z(1:Params.J,:),2); %rows have to add to one anyway

pi_temp=cumsum(pi_z(Params.J+1:2*Params.J,1:Params.J),2);
pi_temp=pi_temp./(max(pi_temp,[],2)*ones(1,Params.J));
...
%% Clean up some space before continuing
clear StationaryDistKron StationaryDist
...
parfor i=1:K*NSims 
    ...
    YoungEarnings(1)=e(s_newborn)*d_grid(Policy(1,a_newborn,s_newborn))*Params.w;
    a_c=a_newborn;
    s_c=s_newborn;
    for t=2:20
        if s_c>Params.J %Check that they are still working
            ...
            YoungEarnings(t)=e(s_c)*d_grid(Policy(1,a_c,s_c))*Params.w;
        end
    ...
    if TheyRetired==0
        for t=21:30
            ...
            if s_c>Params.J %Check that they are still working
                ...
                a_c=d2_c; %a_c=Phi_aprimeKron(d1_c,s_c,new_s_c); %Actually, can ignore the estate tax, since we are only looking at people who don't retire (and so cannot die) anyway
                [~,s_c]=max(cumsumpi_s(s_c,:)>rand(1,1));
                OldEarnings(t-20)=e(s_c)*d_grid(Policy(1,a_c,s_c))*Params.w;
            end
        end
    end
    if TheyRetired==0
...
% One oddity here is that since aging is random, it is possible that 21 to 60 year olds may be retired. 
% We do not wish to allow for this when calculating this statistic. 
% So we only consider people who make it through the 40 periods without retiring.
