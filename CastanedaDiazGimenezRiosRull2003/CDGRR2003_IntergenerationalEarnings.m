function IntergenerationalEarningsCorr=CDGRR2003_IntergenerationalEarnings(NSims,StationaryDist, Policy, Params, simoptions, n_d,n_a,n_z,d_grid,a_grid,z_grid, pi_z)
...
[a2primeIndexes, a2primeProbs]=CreateaprimePolicyInheritanceAsset(Policy,simoptions.aprimeFn, whichisdforinheritasset, n_d, [],n_a, n_z, n_z, gpuArray(d_grid), a_grid, gpuArray(z_grid), gpuArray(z_grid), aprimeFnParamsVec);
...
Policy_aprime=gather(reshape(Policy_aprime,[N_a*N_z*N_zprime,2]));
PolicyProbs=gather(reshape(PolicyProbs,[N_a*N_z*N_zprime,2]));

PolicyProbs_cumsum=cumsum(PolicyProbs,2);

pi_z_cumsum=cumsum(pi_z,2);
...
AvgIndividualEarnings_Generation1=zeros(NSims,1);
AvgIndividualEarnings_Generation2=zeros(NSims,1);

CumSumSteadyStateDistVec_Old=cumsum(reshape(StationaryDistKron(:,Params.J+1:2*Params.J),[N_a*Params.J,1]));
CumSumSteadyStateDistVec_Old=CumSumSteadyStateDistVec_Old./max(CumSumSteadyStateDistVec_Old); %renormalize so it is a cdf

% Free up some memory
clear Policy_a2prime PolicyProbs StationaryDistKron StationaryDist
...
parfor ii=1:NSims
    ...
    temp=ind2sub_homemade([N_a,Params.J],DyingIndex); a_c=temp(1); s_c_minus_J=temp(2);
    s_c=s_c_minus_J+Params.J; %this is to compensate for the fact the DyingIndex is taken only from the old.
    ...
    pi_temp=cumsum(pi_z(s_c,1:Params.J));
    ...
    AvgIndividualEarnings=e(s_newborn)*d_grid(Policy(1,a_newborn,s_newborn))*Params.w;
    currstate=[a_newborn,s_newborn];
    t=1;
    NumOfDeaths=0;
    while NumOfDeaths<2 % i.e. for this generation and the next
        sold_c=s_c;
        t=t+1;
        [~,s_c]=max(pi_z_cumsum(currstate(2),:)>rand(1,1));
        azzprime_c=currstate(1)+N_a*(currstate(2)-1)+N_a*N_z*(s_c-1);
        [~,nprob_c]=max(PolicyProbs_cumsum(azzprime_c,:)>rand(1,1));
        a_c=Policy_aprime(azzprime_c,nprob_c);
        currstate=[a_c,s_c];
        if (Params.J+1<=sold_c && sold_c<=2*Params.J) && s_c<=Params.J % if individual dies
            NumOfDeaths=NumOfDeaths+1;
            if NumOfDeaths==1
                AvgIndividualEarnings_Generation1(ii)=AvgIndividualEarnings;
            else
                AvgIndividualEarnings_Generation2(ii)=AvgIndividualEarnings;
            end
            AvgIndividualEarnings=e(s_c)*d_grid(Policy(1,a_c,s_c))*Params.w;
            t=1;
        elseif s_c<=Params.J % If they are still working update their earnings
            AvgIndividualEarnings=((t-1)/t)*AvgIndividualEarnings+(1/t)*e(s_c)*d_grid(Policy(1,a_c,s_c))*Params.w;
        end
    end
end
AvgIndividualEarnings_Generations=[AvgIndividualEarnings_Generation1, AvgIndividualEarnings_Generation2];

Cov_old_young=sum(AvgIndividualEarnings_Generations(:,1).*AvgIndividualEarnings_Generations(:,2))/NSims-(sum(AvgIndividualEarnings_Generations(:,1))/NSims)*(sum(AvgIndividualEarnings_Generations(:,2))/NSims);
StdDev_old=sqrt(sum(AvgIndividualEarnings_Generations(:,2).^2)/NSims-(sum(AvgIndividualEarnings_Generations(:,2))/NSims)^2);
StdDev_young=sqrt(sum(AvgIndividualEarnings_Generations(:,1).^2)/NSims-(sum(AvgIndividualEarnings_Generations(:,1))/NSims)^2);
